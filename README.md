...
| Group | Commands |
|---|---|
| Server | `PING`, `ECHO`, `TIME`, `INFO` (+ section filter), `COMMAND` (`COUNT` / `LIST` / `INFO`), `HELLO` (+ `AUTH` / `SETNAME`), `CLIENT` (`SETINFO` / `SETNAME` / `GETNAME` / `ID` / `INFO` / `LIST` / `NO-EVICT` / `REPLY` / `TRACKING` / `PAUSE` / `UNPAUSE`), `AUTH`, `WAIT`, `RESET`, `SAVE`, `BGSAVE` (+ `SCHEDULE`), `BGREWRITEAOF`, `LASTSAVE`, `LATENCY` (`RESET` / `HISTORY` / `LATEST` / `GRAPH` / `DOCTOR`), `DEBUG SLEEP`, `MULTI`, `EXEC`, `DISCARD`, `WATCH`, `UNWATCH`, `DBSIZE`, `FLUSHDB`, `FLUSHALL` |
...
Not implemented (PRs welcome): pub/sub, scripting, streams, geo. Transactions (`MULTI` / `EXEC` / `DISCARD` / `WATCH`) return explicit errors — rustyant processes one command per HTTP request with no connection-level state, so cross-request queueing and optimistic CAS cannot be honored honestly; `UNWATCH` returns `+OK` because clearing a never-populated watch set is trivially successful.
...
Working: RESP over HTTP and WebSocket, full string/hash/list/set/zset command dispatch plus `KEYS` / `SCAN` / `HSCAN` / `SSCAN` / `ZSCAN`, server introspection and administration surface (`INFO` / `COMMAND` / `HELLO` / `CLIENT` / `AUTH` / `WAIT` / `SAVE` / `BGSAVE` / `BGREWRITEAOF` / `LASTSAVE` / `LATENCY` / `DEBUG SLEEP` / `MULTI` / `EXEC` / `DISCARD` / `WATCH` / `UNWATCH`), S3-backed storage with per-key TTL and conditional-write CAS on every read-modify-write, 383 Rust tests across 5 suites (18 lib units + 338 HTTP integration + 14 redis-py compat + 6 WebSocket E2E + 7 floci/S3) and 13 Python client tests, structured logs and CloudWatch EMF metrics, CI on GitHub Actions with floci as a service container, SAM template validated in CI.
